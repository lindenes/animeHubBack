...
object CommentQuery:
  ...
  def getCommentList(postId: Int): IO[Json] =
    sql"SELECT comment.*, user.login, user.avatar_path FROM comment INNER JOIN user ON comment.user_id = user.id WHERE comment.post_id = $postId~;"
      ...
      .transact(xa)
      .map(comments =>
        json"""{"Comments": ${comments.map { elem =>
            json"""{"commentId": ${elem.id}, "createdAt":  ${elem
                .createdAt}, "text":  ${elem.text}, "userId": ${elem
                .userId}, "postId":  ${elem.postId}, "userLogin":  ${elem
                .login}, "imagePath":  ${elem.avatarPath}}"""
          }}}"""
      )
      .handleErrorWith(e =>
        IO.pure(
          json"""{"getCommentError":"Ошибка вывода комментариев", "details": ${e
              .toString}}"""
        )
      )
    ...
    sql"SELECT comment.*, user.login, user.avatar_path, post.title FROM comment INNER JOIN user ON comment.user_id = user.id INNER JOIN post ON post.id = comment.post_id WHERE comment.user_id = $personId"
      ...
      .map(comments =>
        comments.map(comment =>
          json"""{"commentId": ${comment.id}, "createdAt":  ${comment
              .createdAt}, "text":  ${comment.text}, "title":${comment
              .title}, "userId": ${comment.userId}, "postId":  ${comment
              .postId}, "userLogin":  ${comment
              .login}, "imagePath":  ${comment.avatarPath}}"""
        )
      )
